param(
    ...
    [string]$ProjectName = "mcpassessment"
)
...
terraform init -input=false `
  -backend-config="bucket=mcpassessment-terraform-state-$awsAccountId" `
  -backend-config="key=$Environment/terraform.tfstate" `
  -backend-config="region=$awsRegion" `
  -backend-config="dynamodb_table=mcpassessment-terraform-locks" `
